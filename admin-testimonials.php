...
if (isset($_POST['testimonial_action'])) {
    ...
    try {
        if ($action === 'delete') {
            ...
            $fileIds = $mediaStatement->fetchAll(PDO::FETCH_COLUMN);
            $delete = $pdo->prepare('DELETE FROM testimonials WHERE id=? RETURNING id');
            $delete->execute([$id]);
            if ($delete->fetchColumn()) foreach ($fileIds as $fileId) imagekit_delete_file($fileId, $config);
...
$loadError = false;
try {
    $query = "SELECT t.id,t.rating,t.message,t.status,t.created_at,t.approved_at,o.order_code,o.customer_name,o.email,o.order_status,COALESCE(json_agg(json_build_object('media_type',tm.media_type,'media_url',tm.media_url,'thumbnail_url',tm.thumbnail_url,'imagekit_file_id',tm.imagekit_file_id,'file_size',tm.file_size,'duration_seconds',tm.duration_seconds,'mime_type',tm.mime_type) ORDER BY tm.media_type) FILTER (WHERE tm.id IS NOT NULL),'[]') AS media_json,COALESCE(string_agg(DISTINCT oi.product_name, ', ' ORDER BY oi.product_name),'') AS products FROM testimonials t INNER JOIN orders o ON o.id=t.order_id LEFT JOIN testimonial_media tm ON tm.testimonial_id=t.id LEFT JOIN order_items oi ON oi.order_id=o.id {$filterSql} GROUP BY t.id,o.id ORDER BY CASE t.status WHEN 'pending' THEN 0 WHEN 'approved' THEN 1 ELSE 2 END,t.created_at DESC";
...
<?php else: ?><div class="testimonial-admin-list"><?php foreach ($testimonials as $testimonial): ?><article class="testimonial-admin-card"><div class="testimonial-admin-media"><?php if (!$testimonial['media']): ?><span style="display:grid;place-items:center;height:100%;min-height:150px;color:#b99586;font:italic 45px Georgia">BL</span><?php else: ?><div class="testimonial-admin-media-set"><?php foreach ($testimonial['media'] as $media): ?><?php if (($media['media_type'] ?? '') === 'image' && !empty($media['media_url'])): ?><a href="<?=admin_h($media['media_url'])?>" target="_blank" rel="noreferrer"><img src="<?=admin_h($media['media_url'])?>" alt="Customer testimonial media" loading="lazy"></a><?php elseif (($media['media_type'] ?? '') === 'video' && !empty($media['media_url'])): ?><video controls preload="metadata" poster="<?=admin_h($media['thumbnail_url'] ?? '')?>"><source src="<?=admin_h($media['media_url'])?>" type="<?=admin_h($media['mime_type'] ?? 'video/mp4')?>"></video><?php endif; ?><?php endforeach; ?></div><?php endif; ?></div><div class="testimonial-admin-copy"><h3><?=admin_h($testimonial['customer_name'])?></h3><div class="testimonial-admin-meta"><?=admin_h($testimonial['email'])?> · Order <?=admin_h($testimonial['order_code'])?> · <?=admin_h($testimonial['order_status'])?><br><?=admin_h($testimonial['products'] ?: 'Order products unavailable')?> · Submitted <?=admin_h(date('j M Y, g:ia', strtotime($testimonial['created_at'])))?></div><div style="margin-top:12px;color:var(--gold);font-size:18px;letter-spacing:2px"><?=str_repeat('★', (int) $testimonial['rating'])?><span style="color:#cfc2bb"><?=str_repeat('★', 5 - (int) $testimonial['rating'])?></span></div><p class="testimonial-admin-message">&ldquo;<?=admin_h($testimonial['message'])?>&rdquo;</p><div class="testimonial-admin-actions"><?php if ($testimonial['status'] !== 'approved'): ?><form method="post" data-loading-form><input type="hidden" name="csrf" value="<?=admin_h($csrfToken)?>"><input type="hidden" name="testimonial_id" value="<?=intval($testimonial['id'])?>"><button class="approve" name="testimonial_action" value="approve">Approve &amp; publish</button></form><?php endif; ?><?php if ($testimonial['status'] !== 'rejected'): ?><form method="post" data-loading-form><input type="hidden" name="csrf" value="<?=admin_h($csrfToken)?>"><input type="hidden" name="testimonial_id" value="<?=intval($testimonial['id'])?>"><button name="testimonial_action" value="reject">Reject</button></form><?php endif; ?><form method="post" data-loading-form onsubmit="return confirm('Delete this testimonial and its uploaded media permanently?');"><input type="hidden" name="csrf" value="<?=admin_h($csrfToken)?>"><input type="hidden" name="testimonial_id" value="<?=intval($testimonial['id'])?>"><button class="delete" name="testimonial_action" value="delete">Delete permanently</button></form></div></div></article><?php endforeach; ?></div><?php endif; ?>
<?php admin_end(); ?>
