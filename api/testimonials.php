...
try {
    ...
    if (isset($input['media']) && $input['media'] !== null) {
        if (!is_array($input['media'])) testimonial_api_error(422, 'The media selection is invalid.');
        $fileId = is_string($input['media']['file_id'] ?? null) ? trim($input['media']['file_id']) : '';
        $mediaType = is_string($input['media']['media_type'] ?? null) ? $input['media']['media_type'] : '';
        if (!$fileId || !in_array($mediaType, ['image', 'video'], true) || !imagekit_configured($config)) testimonial_api_error(422, 'The uploaded media could not be verified.');
        $uploadedFileId = $fileId;
        $scope = testimonial_api_scope($customer, $orderId, $config);
        $media = imagekit_asset_details($fileId, $mediaType, $config, '/boss-lady/testimonials/' . ($mediaType === 'video' ? 'videos' : 'images') . '/' . $scope);
        if (!$media) testimonial_api_error(422, 'The media type, size, duration, or source could not be verified.');
    }
    ...
    if ($media) {
        ...
        $mediaInsert->execute([$testimonialId, $media['media_type'], $media['url'], $media['thumbnail_url'], $media['file_id'], $media['file_size'], $media['duration_seconds'], $media['mime_type']]);
    }
    ...
    if ($uploadedFileId) imagekit_delete_file($uploadedFileId, $config);
